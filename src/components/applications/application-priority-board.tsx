        ...
        {items.map((item) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-400">{item.university}</p>
                <h3 className="text-lg font-semibold text-slate-900">{item.program}</h3>
              </div>
              <span className={`rounded-full px-3 py-1 text-xs font-semibold ${PRIORITY_TONE[item.priority]}`}>
            ...
            <div className="flex items-center gap-6 text-sm text-slate-600">
              ...
              <div>
                ...
                <p className="text-sm">{item.nextDeadline ?? 'TBD'}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Tasks</p>
                <p className="text-sm">{item.tasksRemaining} open</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
              <span className="rounded-full bg-slate-100 px-3 py-1 font-semibold text-slate-700">{item.status}</span>
