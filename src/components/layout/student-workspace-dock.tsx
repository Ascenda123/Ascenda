...
export function StudentWorkspaceDock({ current, metrics }: StudentWorkspaceDockProps) {
  return (
    <motion.section
      className="surface-stage space-y-4"
      initial="hidden"
      whileInView="show"
    ...
    >
      ...
      <motion.div className="relative z-10 grid gap-4 md:grid-cols-2 xl:grid-cols-5" variants={dockStagger}>
        {ITEMS.map((item) => {
          ...
          return (
            <motion.div key={item.key} variants={dockCard}>
              <Link
                href={item.href}
                className={cn(
                  'surface-subcard group flex min-h-[156px] flex-col justify-between p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg',
