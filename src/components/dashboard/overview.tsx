...
export const DashboardOverview = ({ data }: { data: OverviewPayload }) => {
  return (
    <section className="surface-card surface-card--static space-y-6 text-foreground">
      ...
      <div className="grid gap-4 rounded-[24px] border border-border bg-muted/60 p-4 shadow-sm sm:grid-cols-3">
        {[
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {data.highlightCards.map((card) => (
          <div key={card.id} className={cn('rounded-2xl p-4 text-foreground shadow-sm transition-all duration-300 hover:-translate-y-0.5', toneClass(card.tone))}>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.3fr,0.7fr]">
        <div className="rounded-[24px] border border-border bg-card p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_20px_40px_-20px_rgba(15,23,42,0.35)]">
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            {data.steps.map((step) => (
              <div
                key={step.key}
                className="rounded-2xl border border-border bg-muted/50 p-4 shadow-[0_12px_25px_rgba(15,23,42,0.04)] transition-all duration-300 hover:-translate-y-0.5"
              >
                <p className="text-sm font-semibold text-foreground">{step.title}</p>
                <p className="helper-text">{step.description}</p>
        ...
        <div className="rounded-[24px] border border-border bg-card/90 p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_20px_40px_-20px_rgba(15,23,42,0.35)]">
          <p className="text-sm font-semibold text-foreground">Focus radar</p>
          <p className="helper-text">Signals that need your attention right now.</p>
          ...
          {data.averageMatchScore !== null && (
            <div className="mt-4 rounded-2xl border border-border bg-background p-4 text-sm text-muted-foreground shadow-sm transition-all duration-300 hover:-translate-y-0.5">
              ...
              <p className="helper-text">Top programs stay above 80%. Keep refining your profile to lift the average.</p>
            </div>
          )}
        </div>
      </div>
